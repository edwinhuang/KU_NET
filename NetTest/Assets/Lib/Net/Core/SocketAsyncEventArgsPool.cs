...
namespace Kubility
{
		public sealed class SocketEventArgsExtern
		{
		...
		public sealed class SocketAsyncEventArgsPool:SingleTon<SocketAsyncEventArgsPool>
		{
				...
				sealed class SocketAsyncEventArgsPoolTools
				{
						...
						void PoolThread (object obj)
						{
								...
								while (quit == 0) {
//					LogMgr.Log(" ---- SocketAsyncEventArgsPool");
...
//                LogMgr.Log("PoolThread is Done  = " + quit.ToString());
				...
				public SocketEventArgsExtern CreateSocketArgs (IPEndPoint ip, AsyncSocket socket, Action<object, SocketAsyncEventArgs> ev)
				{
						...
						if (Pool.Count < Config.mIns.ARGS_MAX_NUM) {
								...
								if (Pool.Count < Config.mIns.ARGS_MAX_NUM) {
										...
										throw new CustomException ("SocketAsyncEventArgsPool is out of Range .please update the config first.");
						}
